...
@Service
public class CustomSplitService {
    ...
    @Autowired
    CustomSplitService(BalanceRepo balanceRepo, GroupRepo groupRepo, ExpenseRepo expenseRepo){
        CustomSplitService.balanceRepo = balanceRepo;
        CustomSplitService.groupRepo = groupRepo;
        CustomSplitService.expenseRepo = expenseRepo;
    }
    public Double getSplitAmount(Map<Integer,Double> splits, Integer partner){
        return splits.get(partner);
    }
        ...
        if(expense.getGroupId() == null){
            ...
            for(BalanceDao balance : balances){
                for(Integer partner : expense.getExpensePartners()){
                    ...
                    if(balance.getReceiverId().equals(expense.getPaidBy()) && balance.getPayerId().equals(partner)){
                        balance.setBalance(balance.getBalance() + getSplitAmount(expense.getSplit(),partner));
                        try{
            ...
            for(Integer partner : expense.getExpensePartners()){
                if(updated.get(partner) == null){
                    BalanceDao balanceDao = new BalanceDao(partner, expense.getPaidBy(), getSplitAmount(expense.getSplit(),partner));
                    try{
            ...
            if(optionalGroupDao.isPresent()){
                ...
                for(Integer partner : expense.getExpensePartners()){
                    if(updated.get(partner) == null){
                        BalanceDao balanceDao = new BalanceDao(partner, expense.getPaidBy(), getSplitAmount(expense.getSplit(),partner),expense.getGroupId());
                        try{
                            balanceRepo.save(balanceDao);
                        }catch (Exception e){
                            System.out.println(e.getMessage());
